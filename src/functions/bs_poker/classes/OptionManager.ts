	...
	public constructor({ options }: ChatInputCommandInteraction<"cached">) {
		...
		if (maxPlayerLimit < 2) {
			throw new OptionCreationError(
				`With your current settings, the deck is not large enough to allow for a game of BS Poker.`
			);
		}

		if (maxPlayerLimit > 15) maxPlayerLimit = 15;
		this.playerLimit =
...
Please decrease the player limit to a value less than or equal to ${maxPlayerLimit}.`
		...
		if (this.trueInsuranceCount > 4) {
			throw new OptionCreationError(
				`The number of insurance cards in the deck is greater than 4. (Enabling insurance specials adds 2 insurance cards.)`
			);
		}

		if (this.useSpecialCards && this.jokerCount > 2) {
			throw new OptionCreationError(
				`The number of jokers in the deck is greater than 4. (Enabling special cards adds 2 jokers.)`
			);
		}

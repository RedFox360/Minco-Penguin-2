...
const team = new SlashCommand()
    ...
    .addSubcommand(subcommand => subcommand
    .setName("invite")
    .setDescription("Invite someone outside of the game to join your team")
    .addUserOption(option => option
    .setName("player")
    ...
    if (subcommand === "join") {
        ...
        if (teamWithAsker) {
            await interaction.reply({
                content: "You are already in a team or a player in the current game. If you are a player, use </team invite:1252747326473109631> to add someone to your team.",
                ephemeral: true,
            });
            return;
        }
        ...
        msg
            .awaitMessageComponent({
            componentType: ComponentType.Button,
            filter: bi => bi.user.id === player.id &&
        ...
        if (player.id === interaction.user.id) {
            await interaction.reply({
                content: "You may not kick yourself from your team. Try using </team leave:1252747326473109631>.",
                ephemeral: true,
            });
            return;
        }

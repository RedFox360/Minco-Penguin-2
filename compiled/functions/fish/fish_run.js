...
import { colors, hasAdminForGames, msToRelTimestamp, removeByValue, removeC, shuffleArray, } from "../util.js";
...
const FishRun = new Subcommand()
    ...
    .setDescription("All messages by players are deleted 2 minutes after they being sent (Default: true)")
    .setRequired(false)))
    .setRun(async (interaction) => {
    const deleteHistory = interaction.options.getBoolean("delete_history") ?? true;
    if (interaction.guild.id === process.env.MAIN_GUILD_ID &&
        deleteHistory &&
        !interaction.channel.name.toLowerCase().startsWith("fish")) {
        await interaction.reply({
            content: 'In the HCH, Minco Penguin 2, can only play fish games in channels/threads that start with "fish" when `delete_history` is enabled.',
            ephemeral: true,
        });
        return;
    }
...
Team 2: ${formatTeam(team1)}`)
    ...
    collector.on("end", (_, reason) => {
        ...
        const game = new Fish(interaction.channel, interaction.user.id, team0, team1, deleteHistory);
        game.gameLogic();
    });
});
export default FishRun;
//# sourceMappingURL=fish_run.js.map
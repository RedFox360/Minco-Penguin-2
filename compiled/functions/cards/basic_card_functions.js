...
export function valueToSymbol(value, short = false) {
    if (value == null)
        return null;
    switch (value) {
        case 1:
...
export function cardToEmoji(card) {
    ...
    if (card.value === 15) {
        ...
        if (card.suit === "rj")
            return [newEmojiSuits.insurance, newEmojiSuits.red];
...
export function formatCardSideways(card, short = false) {
    if (card.suit === "bj") {
        ...
        if (card.value === 15)
            return short ? "BI" : "Black Insurance";
    }
    if (card.suit === "rj") {
        if (card.value === 1)
            return short ? "RX" : "Red Joker";
        if (card.value === 15)
            return short ? "RI" : "Red Insurance";
    }

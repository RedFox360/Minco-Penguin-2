...
function flushCallToSuit(call) {
    if (namesHas(RNI[HandRank.Flush], call))
        return "H";
    if (namesHas(RNI[HandRank.Flush + 1], call))
        return "D";
    if (namesHas(RNI[HandRank.Flush + 2], call))
        return "C";
    if (namesHas(RNI[HandRank.Flush + 3], call))
        return "S";
    return null;
}
function flushCallsToSuits(...calls) {
    const suits = [];
...
const validSymbol = (x) => symbolToValue(x) != null;
...
export function callsEqual(call1, call2) {
    ...
    if (call1.call === HandRank.DoubleFlush) {
        ...
        return arraysEqual(high1, high2, (a, b) => a.value === b.value && a.suit === b.suit);
    }
    if (call1.call === HandRank.Flush) {
        const call2High = call2.high;
...
function jokersInDeck(deck) {
    return countInArray(deck, card => card.value === 1);
}
export function callInDeck(call, deck, gInsurances, gJokers) {
    ...
    if (call.call === HandRank.DoublePair)
        return (countInArray(deck, card => card.value === call.high[0]) >= 2 &&
    ...
    if (call.call === HandRank.DoubleFlush) {
        ...
        if (flush1.value === 15 && flush2.value === 15) {
            ...
            return flushCall.high.every(card => {
                return (countInArray(deck, c => c.suit === card.suit && c.value < card.value) >= 3);
            });
        }
    ...
    if (deck.length >= 5) {
        ...
        for (let value = startingValue; value >= 1; value--) {
            for (const suit of suits) {
                ...
                if (straight.every(value => groupsObj[value]?.some(card => card.suit === suit))) {
                    return {
            ...
            for (const suit1 of suits) {
                for (let j = 2; j <= 15; j++) {
                    for (const suit2 of suits) {
                        ...
                        if (callInDeck(dfCall, deck, insurances, jokers))
                            return dfCall;
                    }
                }
            }

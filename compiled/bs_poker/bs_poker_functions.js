...
import { HandRank, names, RNI, RNIKeys, emoji, newEmoji, newEmojiSuits, suits, royalFlushes, } from "./bs_poker_types.js";
function suitToBasicEmoji(suit) {
    switch (suit) {
        case "H":
            return emoji.hearts;
        case "D":
...
function valueToSymbol(value, short = false) {
    if (value == null)
        return null;
    switch (value) {
        case 1:
...
function cardToEmoji(card) {
    if (card.value === 1) {
        if (card.suit === "bj")
            return [newEmojiSuits.joker, newEmojiSuits.black];
        if (card.suit === "rj")
            return [newEmojiSuits.joker, newEmojiSuits.red];
    ...
    if (!cardEmojis)
        return null;
    if (card.suit === "H")
        return [cardEmojis[1], newEmojiSuits.hearts];
    if (card.suit === "D")
        return [cardEmojis[1], newEmojiSuits.diamonds];
    if (card.suit === "C")
        return [cardEmojis[0], newEmojiSuits.clubs];
    if (card.suit === "S")
        return [cardEmojis[0], newEmojiSuits.spades];
    return null;
}
export function formatDeckLines(deck) {
    const line1 = [];
    const line2 = [];
...
export function collectorEnd(collector) {
    return new Promise(resolve => collector.on("end", () => {
        resolve();
    }));
}
...
function symbolToValue(textGiven) {
    ...
    if (text === "joker" || text === "x")
        return 1;
    if (text === "insurance" || text === "i")
        return 15;
    if (text === "k" || text === "king")
        return 13;
    if (text === "q" || text === "queen")
        return 12;
    if (text === "j" || text === "jack" || text === "knave")
        return 11;
    if (text === "a" || text === "ace")
        return 14;
    if (text === "deuce")
...
function straightHighToCards(high) {
    if (high === 15)
        return [15, 14, 13, 12, 11];
    if (high === 1)
        return [1, 14, 13, 12, 11];
    const cards = [];
    for (let i = 0; i < 5; i++) {
        cards.push((((high - 1 - i + 13) % 13 || 13) + 1));
    }
    return cards;
}
export async function highestCallInDeck(deck) {
    ...
    if (deck.length >= 5) {
        for (let value = 14; value >= 2; value--) {
            for (const suit of suits) {
                const straight = straightHighToCards(value);
    ...
    if (deck.length >= 5) {
        ...
        // Straight
        for (let i = 15; i >= 1; i--) {
            const straight = straightHighToCards(i);
    ...
    for (let i = 15; i >= 1; i--) {
        if (deck.filter(card => card.value === i).length >= 2) {
            return { high: { value: i, suit: null }, call: HandRank.Pair };
        }
    }
    // High
...
export function replyThenDelete(message, text) {
    message
        .reply({
        content: text,
    })
...
export function spliceRandom(arr, count) {
    const spliced = [];
...
export function timeToRelativeTimestamp(timeMS) {
